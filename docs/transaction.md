...
Note also that an output can be used only once. That's why there's a concept of "change address" in the ravencoin ecosystem: if an output of 10 RVN is available for me to spend, but I only need to transmit 1 RVN, I'll create a transaction with two outputs, one with 1 RVN that I want to spend, and the other with 9 RVN to a change address, so I can spend this 9 RVN with another private key that I own.
...
var transaction = new Transaction().fee(1e8);  // Generous fee of 1 RVN
```
